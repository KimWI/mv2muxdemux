...
class MV2MasterEncoder:
    ...
    # ==========================================================
    # 2. 고급 팔레트 추출 (Edge + ROI 집중 가중치 + Anchoring)
    # ==========================================================
    def _extract_palette(self, img_array):
        pixels = img_array.reshape(-1, 3)
        h, w, _ = img_array.shape
        
        # [전략 1] Canny 엣지 픽셀 추출 (윤곽선 보호)
        gray = cv2.cvtColor(img_array, cv2.COLOR_RGB2GRAY)
        edges = cv2.Canny(gray, threshold1=50, threshold2=150)
        edge_pixels = img_array[edges == 255]
        
        # [전략 2] 화면 중앙(ROI) 픽셀 집중 추출 (격투 게임 캐릭터 집중 보호)
        # 가로 25%~75%, 세로 15%~90% 영역을 주인공 영역으로 간주합니다.
        roi_y1, roi_y2 = int(h * 0.15), int(h * 0.90)
        roi_x1, roi_x2 = int(w * 0.25), int(w * 0.75)
        roi_pixels = img_array[roi_y1:roi_y2, roi_x1:roi_x2].reshape(-1, 3)
        
        # [전략 3] 통계 조작 (Weighting)
        # 원본 + (엣지 5배) + (화면 중앙 캐릭터 영역 10배)로 배열을 강제로 부풀립니다.
        components = [pixels]
        if len(edge_pixels) > 0:
            components.extend([edge_pixels] * 5)
        if len(roi_pixels) > 0:
            components.extend([roi_pixels] * 10)
            
        weighted_pixels = np.vstack(components)
        unique_colors = np.unique(weighted_pixels, axis=0)
        
        target_colors = 14 # 1자리는 검은색 예약
        raw_pal = []
        ...
        if len(unique_colors) <= target_colors:
            ...
            # Pillow의 C-Engine에 조작된 가중치 배열을 먹임
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("사용법: python mv2_encoder_roi.py [입력.mp4] [출력.mv2] [알고리즘: kmeans/mediancut/octree]")
        sys.exit(1)
